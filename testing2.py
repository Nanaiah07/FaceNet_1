...
# Measure time before loading the FACELOADING class
start_time_load_class = time.time()
...
class FACELOADING:
    def __init__(self, directory, min_face_size=20, max_face_size=None):
        ...
        self.image_counter = 0  # Counter to track the number of tested images
...
# Measure time after loading the FACELOADING class
end_time_load_class = time.time()

# Calculate the time taken to load the class
print(f"Time taken to load FACELOADING class: {end_time_load_class - start_time_load_class:.2f} seconds")

# Print the number of images tested
print(f"Number of images tested: {faceloading.image_counter}")
